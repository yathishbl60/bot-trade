...
class Strategies:
    ...
    def ma_strategy(df, i: int):
        ...
        if not df.__contains__('slow_sma'):
            Indicators.add_indicator(df, indicator_name="sma", col_name="slow_sma", args=30)
    ...
    def boll_strategy(df, i: int):
        ...
        if not df.__contains__('low_boll'):
            Indicators.add_indicator(df, indicator_name="lbb", col_name="low_boll", args=14)
    ...
    def ichimoku_bullish(df, i: int):
        ...
        if not df.__contains__('tenkansen') or not df.__contains__('kijunsen') or \
            ...
            Indicators.add_indicator(df, indicator_name="ichimoku", col_name=None, args=None)
